 ...
 * @license MIT
 *
 * @see  https://github.com/justjim1220/MMM-Win-Voice
 */

/**
 * @external pocketsphinx-continuous
 * @see https://github.com/justjim1220/pocketsphinx-continuous-node
 */
...
module.exports = NodeHelper.create({
    ...
    checkFiles() {
        ...
        fs.stat('modules/MMM-Win-Voice/words.json', (error, stats) => {
            if (!error && stats.isFile()) {
                fs.readFile('modules/MMM-Win-Voice/words.json', 'utf8', (err, data) => {
                    if (!err) {
    ...
    generateDicLM() {
        ...
        fs.writeFile('modules/MMM-Win-Voice/words.json', JSON.stringify({ words: this.words }), (err) => {
            if (err) {
        ...
        lmtool(this.words, (err, filename) => {
            if (err) {
                ...
                fs.renameSync(`${filename}.dic`, 'modules/MMM-Win-Voice/MMM-Win-Voice.dic');
                fs.renameSync(`${filename}.lm`, 'modules/MMM-Win-Voice/MMM-Win-Voice.lm');
     ...
     */
    logDebug(data) {
        fs.appendFile('modules/MMM-Win-Voice/debug.log', data, (err) => {
            if (err) {
    ...
    logError(error) {
        if (error) {
            fs.appendFile('modules/MMM-Win-Voice/error.log', `${error}\n`, (err) => {
                if (err) {
                    console.log(`${this.name}: Couldn't save error to log file!`);
                }
